...
function StaffDashboard({ user }) {
  ...
  const hasElevatedQueueTabs = ["fund_manager", "admin", "super_admin"].includes(user.app_role);

  const pendingQueueUrl = hasElevatedQueueTabs
    ? `${createPageUrl("Queue")}?view=all_org_pending`
    : createPageUrl("Queue");

  const approvedQueueUrl = `${createPageUrl("Queue")}?view=my_funds&status=Approved`;
  const reportsUrl = createPageUrl("Reports");
  const fundsUrl = createPageUrl("Funds");
      ...
      {permissions.view_stats !== false && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard
            ...
            icon={Clock}
            color="amber"
            onClick={() => navigate(pendingQueueUrl)}
            aria-label={`Pending review: ${stats.pendingReview}. Open in review queue.`}
          />
          <StatCard
            title="Approved"
            ...
            icon={CheckCircle}
            color="emerald"
            {...(hasElevatedQueueTabs
              ? {
                  onClick: () => navigate(approvedQueueUrl),
                  "aria-label": `Approved requests: ${stats.approved}. Open in review queue.`,
                }
              : {})}
          />
          <StatCard
            ...
            aria-label="Total disbursed. Open reports."
          />
          <StatCard
            title="Active Funds"
            ...
            {...(user.app_role === "fund_manager"
              ? {
                  onClick: () => navigate(fundsUrl),
                  "aria-label": `Active funds: ${stats.activeFunds}. Open funds list.`,
                }
              : {})}
          />
        </div>
        ...
        {permissions.view_pending_requests !== false && (
          ...
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            ...
            <Button variant="ghost" size="sm" asChild>
              <Link to={createPageUrl("Queue")} className="text-indigo-600 dark:text-indigo-400">
        ...
        {permissions.view_fund_overview !== false && (
          ...
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            ...
            {user.app_role !== "reviewer" && (
              <Button variant="ghost" size="sm" asChild>
                <Link to={createPageUrl("Funds")} className="text-indigo-600 dark:text-indigo-400">
                  Manage
                </Link>
              </Button>
            )}
